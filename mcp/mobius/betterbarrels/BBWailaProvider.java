...
public class BBWailaProvider implements IWailaDataProvider {
	...
	public List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,	IWailaConfigHandler config) {
		...
		currenttip.add(StatCollector.translateToLocalFormatted("text.jabba.waila.structural", tebarrel.coreUpgrades.levelStructural));
		currenttip.add(StatCollector.translateToLocalFormatted("text.jabba.waila.upgrades", tebarrel.coreUpgrades.getFreeSlots(), tebarrel.coreUpgrades.getMaxUpgradeSlots()));		
		
		if (barrelStack != null){
			if(config.getConfig("bb.itemtype"))
				currenttip.add(barrelStack.getDisplayName());
			if(config.getConfig("bb.itemnumb"))
					currenttip.add(StatCollector.translateToLocalFormatted("text.jabba.waila.items", tebarrel.getStorage().getAmount(), tebarrel.getStorage().getItem().getMaxStackSize() * tebarrel.getStorage().getMaxStacks()));
			if(config.getConfig("bb.space"))
					currenttip.add(StatCollector.translateToLocalFormatted("text.jabba.waila.stacks", tebarrel.getStorage().getMaxStacks()));			
		} else {
			if(config.getConfig("bb.itemtype"))
