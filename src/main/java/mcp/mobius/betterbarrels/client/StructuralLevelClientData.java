...
public class StructuralLevelClientData {
	...
	public void cacheStackAndName() {
		BetterBarrels.debug("15 - Looking up user friendly name for " + (level.material.isOreDict() ? level.material.name : (level.material.modDomain + ":" + level.material.name + ":" + level.material.meta)));
	...
	public static void loadBaseTextureData() {
		...
		StructuralLevelClientData.baseTexturePixels.labelBorder = getPixelsForTexture(false, BetterBarrels.modid + ":barrel_label_border");
		StructuralLevelClientData.baseTexturePixels.labelBackground = getPixelsForTexture(false, BetterBarrels.modid + ":barrel_label_background");
		StructuralLevelClientData.baseTexturePixels.topBorder = getPixelsForTexture(false, BetterBarrels.modid + ":barrel_top_border");
		StructuralLevelClientData.baseTexturePixels.topBackground = getPixelsForTexture(false, BetterBarrels.modid + ":barrel_top_background");
		StructuralLevelClientData.baseTexturePixels.topLabel = getPixelsForTexture(false, BetterBarrels.modid + ":barrel_top_label");
		StructuralLevelClientData.baseTexturePixels.sideBorder = getPixelsForTexture(false, BetterBarrels.modid + ":barrel_side_border");
		StructuralLevelClientData.baseTexturePixels.sideBackground = getPixelsForTexture(false, BetterBarrels.modid + ":barrel_side_background");
		StructuralLevelClientData.baseTexturePixels.item = getPixelsForTexture(true, BetterBarrels.modid + ":capaupg_base");
		StructuralLevelClientData.baseTexturePixels.itemArrow = getPixelsForTexture(true, BetterBarrels.modid + ":capaupg_color");
	}
	...
	private static class AccessibleTextureAtlasSprite extends TextureAtlasSprite {
		...
		public void replaceTextureData(int[] pixels, int mipmapLevels) throws Exception {
			...
			aint = (int[][])AccessibleTextureAtlasSprite.setupAnisotropic.invoke(this, aint, useAnisotropic ? this.width - 16: this.width, useAnisotropic ? this.height - 16: this.height);
			aint = TextureUtil.generateMipmapData(mipmapLevels, this.width, aint);
			BetterBarrels.debug("37 - Attempting to replace texture for ["+this.getIconName()+"] with an array of ["+(aint!=null?aint[0].length:"(null)")+"] pixels, current texture dims are ["+this.width+"x"+this.height+"] for a total size of "+(this.width*this.height));
			BetterBarrels.debug(this.toString());
			if (aint[0].length != (this.height * this.width)) {
				throw new Exception("Attempting to replace texture image data with " + (aint[0].length > (this.height * this.width) ? "too much": "too little") + " data.");
			}
	...
	private static StructuralLevelClientData.AccessibleTextureAtlasSprite registerIcon(IIconRegister par1IconRegister, String key) {
		TextureMap texmap = (TextureMap)par1IconRegister;
		StructuralLevelClientData.AccessibleTextureAtlasSprite ret = new AccessibleTextureAtlasSprite(key, texmap.getTextureType());
	...
	public void registerItemIcon(IIconRegister par1IconRegister, int ordinal) {
		this.iconItem = StructuralLevelClientData.registerIcon(par1IconRegister, BetterBarrels.modid + ":blanks/capacity/" + String.valueOf(ordinal));
	}

	public void registerBlockIcons(IIconRegister par1IconRegister, int ordinal) {
		this.iconBlockSide = StructuralLevelClientData.registerIcon(par1IconRegister, BetterBarrels.modid + ":barrel_side_" + String.valueOf(ordinal));
		this.iconBlockTop = StructuralLevelClientData.registerIcon(par1IconRegister, BetterBarrels.modid + ":barrel_top_" + String.valueOf(ordinal));
		this.iconBlockLabel = StructuralLevelClientData.registerIcon(par1IconRegister, BetterBarrels.modid + ":barrel_label_" + String.valueOf(ordinal));
		this.iconBlockTopLabel = StructuralLevelClientData.registerIcon(par1IconRegister, BetterBarrels.modid + ":barrel_labeltop_" + String.valueOf(ordinal));
	}
				...
				try {
					while(level.material.meta >= 0) {
						...
						if (materialBlock != Blocks.air && !materialBlock.getUnlocalizedName().equalsIgnoreCase("tile.ForgeFiller")) {
							...
							materialPixels = getPixelsForTexture(false, materialBlock.getIcon(0, materialStack.getItemDamage()));
							...
							materialPixels = getPixelsForTexture(true, materialItem.getIconFromDamage(materialStack.getItemDamage()));
							foundSourceMaterial = true;
					...
					if (!foundSourceMaterial) {
						BetterBarrels.log.error("Encountered an issue while locating the requested source material[" + (level.material.isOreDict() ? level.material.name : (level.material.modDomain + ":" + level.material.name + ":" + level.material.meta)) + "].  Ore Dictionary returned " + materialStack.getUnlocalizedName() + " as the first itemStack for that request.");
					} else {
